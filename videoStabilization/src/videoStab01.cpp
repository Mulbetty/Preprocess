/**
 * 通过使用opencv自带的去抖动算法，实现视频稳像。参考链接：https://blog.csdn.net/weixin_45250844/article/details/103447430
 * 
 */
...
void cacStabVideo(cv::Ptr<cv::videostab::IFrameSource> stabFrames, std::string srcVideoFile)
{
	try
	{
		cv::Ptr<cv::videostab::VideoFileSource> srcVideo
			= cv::makePtr<cv::videostab::VideoFileSource>(srcVideoFile);
...
int stabilize01(std::string inputPath_, std::string outputPath_)
{
	...
	std::string subfix = "_stab1_output.mp4";
	...
	clock_t end = clock();
	std::cout << "videoStabilization 01 用时：" << end - start << std::endl;
	stabFrames.release();
	return 0;
}